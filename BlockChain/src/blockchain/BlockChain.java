...
public class BlockChain implements Serializable {
	...
	public static BlockChain load(String fileName) throws IOException, ClassNotFoundException, NoSuchAlgorithmException,
		...
		synchronized (blocks) {
			if (!blocks.empty()) {
				...
				for (int i = 1; i < blocks.size(); i++) {
					...
					if (previousBlockMessages.messages().mapToInt(m -> m.getId()).max()
							.orElse(-1) >= currentBlockMessages.messages().mapToInt(m -> m.getId()).min().orElse(0)) {
	...
	public void queueMessage(Message message) {
		synchronized (messagesForTheNextBlock) {
			int lastMessageId = messagesForTheNextBlock.stream().mapToInt(m -> m.getId()).max().orElse(0);
	...
	public boolean tryPutNewBlock(int minerId) throws IOException {
		...
		synchronized (messagesForTheNextBlock) {
			var block = new Block(minerId, blockId, proofLength, previous,
					Integer.compare(proofLength, previousProofLength), messagesForTheNextBlock.isEmpty() ? null
							: new MessageList(messagesForTheNextBlock.stream().collect(toList())));
